                ...
                <div class="panel-body">
                    <div class="record-info">
                        ...
                        $form->field($model, 'thumbnail')->widget(yeesoft\media\widgets\FileInput::className(), [
                            ...
                            'template' => '<div class="works-thumbnail thumbnail"></div><div class="input-group">{input}<span class="input-group-btn">{button}</span></div>',
                            'thumb' => $this->context->module->thumbnailSize,
                            'imageContainer' => '.works-thumbnail',
                            'pasteData' => yeesoft\media\widgets\FileInput::DATA_URL,
                            'callbackBeforeInsert' => 'function(e, data) {
                                $(".works-thumbnail").show();
                            }',
                        ])
                        ?>
                    </div>
...
$js = <<<JS
    var thumbnail = $("#works-thumbnail").val();
    console.log('hi');
    if(thumbnail.length == 0){
        $('.works-thumbnail').hide();
    } else {
        $('.works-thumbnail').html('<img src="' + thumbnail + '" />');
    }
JS;
