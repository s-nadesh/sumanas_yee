                ...
                <div class="panel-body">
                    <div class="record-info">
                        ...
                        $form->field($model, 'thumbnail')->widget(yeesoft\media\widgets\FileInput::className(), [
                            ...
                            'template' => '<div class="clients-thumbnail thumbnail"></div><div class="input-group">{input}<span class="input-group-btn">{button}</span></div>',
                            'thumb' => $this->context->module->thumbnailSize,
                            'imageContainer' => '.clients-thumbnail',
                            'pasteData' => yeesoft\media\widgets\FileInput::DATA_URL,
                            'callbackBeforeInsert' => 'function(e, data) {
                                $(".clients-thumbnail").show();
                            }',
                        ])
                        ?>
                    </div>
...
$js = <<<JS
    var thumbnail = $("#clients-thumbnail").val();
    console.log('hi');
    if(thumbnail.length == 0){
        $('.clients-thumbnail').hide();
    } else {
        $('.clients-thumbnail').html('<img src="' + thumbnail + '" />');
    }
JS;
